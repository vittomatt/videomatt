...
@injectable()
export class SQSUserEventConsumer extends SQSEventConsumer {
    constructor(
        @inject(TOKEN.SHARED.SQS_CLIENT) protected readonly sqsClient: SQSClient,
        @inject(TOKEN.USER.SQS_QUEUE_URL) protected readonly queueUrl: string,
        @inject(TOKEN.SHARED.LOGGER) protected readonly logger: Logger,
        @inject(TOKEN.USER.HANDLER) protected readonly handler: Handler
    ) {
